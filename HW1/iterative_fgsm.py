...
for eps_try in [0.1, 0.2, 0.5, 1.0, 1.5, 2.0, 3.0, 5.0]:
    for alpha_try in [0.01, 0.05, 0.1, 0.2]:
        print(f"\nTrying epsilon = {eps_try}, alpha = {alpha_try}")
        eps = eps_try - 1e-7
        
        iterations = 200
        
        # Step 1: Start with x'_0 = x
        adv_x = x
        
        for i in range(iterations):
            adv_x.requires_grad_()
            
            # Compute loss with new input
            loss = L(N(adv_x), torch.tensor([t], dtype=torch.long))
            loss.backward()
            
            # Step 2: Compute perturbation eta
            eta = alpha_try * torch.sign(adv_x.grad)
            
            # Step 3: Add perturbation
            with torch.no_grad():
                adv_x = adv_x - eta
            
            # Step 4: Check if result class is t
            if N(adv_x).argmax(dim=1).item() == t:
                print(f"Success at iteration {i+1} with epsilon {eps_try}, alpha {alpha_try}!")
                success = True
...
if success:
    adv_x = best_adv_x
    epsReal = best_eps
    print(f"\nFinal success with epsilon = {best_eps}, alpha = {best_alpha}")
    print("Working adv_x:", adv_x)
else:
    print("Failed, attack does not work!!!!")
